...
def generate_croissant_metadata(dataset_name, description, file_path, num_records, source_url=None, source_file=None, llm_model="gpt-oss:latest", mapping_file=None, hips_code=None):
    ...
    # Load Mappings
    if not mapping_file:
         # Try to find mappings in the package structure
         base_dir = os.path.dirname(os.path.abspath(__file__))
         
         # 1. Check if mappings is a sibling directory (dev mode or flat layout)
         candidate = os.path.join(os.path.dirname(base_dir), "mappings", "csv_to_croissant.ttl")
         if not os.path.exists(candidate):
             # 2. Check if mappings is inside the package (installed mode)
             candidate = os.path.join(base_dir, "mappings", "csv_to_croissant.ttl")
